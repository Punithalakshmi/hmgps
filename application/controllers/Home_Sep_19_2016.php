...
class Home extends CI_Controller {
	...
	public function search($search='',$ajax=0)
	{
		try{
			...
			if(isset($_POST['search'])){
                ...
                delete_cookie('trackeduser');
                delete_cookie('trackinguser');
                delete_cookie('trackmapID');
                delete_cookie('trackedUser_position');
            ...
            //if any user already tracked or not, if not means need to track initial search map
            $tracked_user         = get_cookie("trackeduser");
            $tracking_user        = get_cookie("trackinguser");
            $tracked_channel_id   = get_cookie("trackmapID");
            $tracked_user_position= get_cookie("trackedUser_position");

            $current_tracked_user = '';
	                ...
	                if(isset($_POST['search'])){
	                   ...
	                   if($user_type == 'admin') {
                          ...
                          $trackmapId  = array(
    				        ...
    				        set_cookie($trackmapId);  
                            
                            $tracked_user_pos = $val->user->position->lat."_".$val->user->position->lon."_"."20";
                            $tracked_user_position = array(
                                    						'name' => 'trackedUser_position',
                                    						'value' => $tracked_user_pos,
                                    						'expire' => time()+86400,
                                    						'path'   => '/'
                        						         );
    				        set_cookie($tracked_user_position);
	   				...
	   				$str    = '<div id="seach_content">
							...
							<ul class="contact_card">
                             ...
                             if($this->data['user_info']['channel_id'] == $channel_id) {
                              ...
                              $tracked_user = (empty($current_tracked_user))?get_cookie("trackeduser"):$current_tracked_user;  
                              ...
                              $str    .='<li><input type="radio" name="breadcrumb" class="breadcrumb" onclick="breadcrumb();" data-uid="'.$val->user->profile->id.'" />10 mins</li>';
                              $str    .='<li><input type="radio" name="breadcrumb" class="breadcrumb" onclick="breadcrumb();" data-uid="'.$val->user->profile->id.'" />24 Hours</li>';
							  $str    .= '</ul></div>';
   ...
   function breadcrumb()
   {
            ...
            $this->service_param['user_id']     = $user_id;
            $this->service_param['time_limit']  = 1;
